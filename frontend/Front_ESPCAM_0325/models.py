...
class DBManager:
    ...
    # 불량 검출 결과 저장 메서드 (추가)
    def insert_defect_result(self, result):
        try:
            self.connect()
            sql = """
                INSERT INTO defect_results (date, score, overlay, created_at)
                VALUES (%s, %s, %s, NOW())
            """
            self.cursor.execute(sql, (result['date'], result['score'], result['overlay']))
            self.connection.commit()
            print(f"불량 검출 결과 저장 완료: {result['date']}")
            return True
        except mysql.connector.Error as error:
            print(f"DB 오류 발생: {str(error)}")
            return False
